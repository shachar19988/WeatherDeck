...
- A session can be booked past the models' reach. `suitableHours` returns null for "no forecast" and never records it as a baseline, which is what lets the first forecast be announced when the day comes into range. `hadForecast` on the session says whether that announcement is news — a day booked inside the range already showed its count when saved.
- A daily summary covers everything in the diary, changed or not, guarded by date and confined to waking hours. The per-session alerts fire only on a change, which is right for interruptions but leaves no way to ask what next week looks like.
...
- Widget text is written for a 2x2, which is about 130dp of usable width — roughly fourteen characters at 14sp bold. Give each reading its own line rather than packing two onto one; wind and sea shared a line for a while and the sea was simply never visible. Anything longer is silently cut, and what falls off the end is whatever was written last — which is why the sea used to look missing when it was there all along. Keep `ellipsize` on every widget TextView so overflow is at least visible.
- Sheets are dismissed by dragging the handle or title, never the body: a panel that sometimes scrolls and sometimes closes under the same gesture is worse than one that never closes. `useSheetDrag` finds the sheet from the event rather than a ref — a ref argument cannot be written through and a returned one is read during render, and the compiler refuses both.
- The day strip follows the table's scroll as well as driving it. A chip lit for a day that is not under the reader's thumb is simply wrong.
- The widget shows today and only today. A planned-session note was tried underneath and removed: it did not earn its place, and the width it took was what the sea needed.
