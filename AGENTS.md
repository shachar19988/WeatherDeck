...
- Widget text is written for a 2x2, roughly twenty characters. Anything longer is silently cut, and what falls off the end is whatever was written last — which is why the sea used to look missing when it was there all along. Keep `ellipsize` on every widget TextView so overflow is at least visible.
